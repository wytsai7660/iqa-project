"""
Training script for Distortion Classification task.

This script trains only the distortion classification component of the multi-task IQA model.
Memory-efficient approach: Only loads distortion task data, avoiding OOM issues.

Note: Distortion messages include scene context if available in the dataset.

Usage:
    uv run -m src.new_train.train_distortion \
        --dataset_paths datasets/bid/ \
        --output_dir outputs/distortion_classifier \
        --num_train_epochs 3
"""
...
def collate_fn_distortion(batch):
    """
    Collate function for distortion classification task only.
    Only processes distortion-related data to save memory.
    """
    import torch
    ...
    # Extract distortion data from batch
    distortion_input_ids_A = [item["input_ids_distortion_A"] for item in batch]
    distortion_attention_mask_A = [item["attention_mask_distortion_A"] for item in batch]
    distortion_labels_A = [item["labels_distortion_A"] for item in batch if "labels_distortion_A" in item]
    distortion_input_ids_B = [item["input_ids_distortion_B"] for item in batch]
    distortion_attention_mask_B = [item["attention_mask_distortion_B"] for item in batch]
    distortion_labels_B = [item["labels_distortion_B"] for item in batch if "labels_distortion_B" in item]
    
    # Pad and stack distortion sequences
    input_ids_distortion_A = pad_and_stack(distortion_input_ids_A, pad_value=0)
    attention_mask_distortion_A = pad_and_stack(distortion_attention_mask_A, pad_value=0)
    input_ids_distortion_B = pad_and_stack(distortion_input_ids_B, pad_value=0)
    attention_mask_distortion_B = pad_and_stack(distortion_attention_mask_B, pad_value=0)
    
    # Create proper labels from raw labels (0=ignore, 1=train)
    if distortion_labels_A:
        raw_labels_A = pad_and_stack(distortion_labels_A, pad_value=0)
        # Convert: label=0 → -100 (ignore), label=1 → token_id (train)
        labels_distortion_A = torch.where(raw_labels_A == 0, torch.tensor(-100, dtype=input_ids_distortion_A.dtype), input_ids_distortion_A)
        labels_distortion_A[attention_mask_distortion_A == 0] = -100  # Also mask padding
    else:
        # Fallback: use input_ids (old behavior)
        labels_distortion_A = input_ids_distortion_A.clone()
        labels_distortion_A[attention_mask_distortion_A == 0] = -100
    
    if distortion_labels_B:
        raw_labels_B = pad_and_stack(distortion_labels_B, pad_value=0)
        labels_distortion_B = torch.where(raw_labels_B == 0, torch.tensor(-100, dtype=input_ids_distortion_B.dtype), input_ids_distortion_B)
        labels_distortion_B[attention_mask_distortion_B == 0] = -100
    else:
        labels_distortion_B = input_ids_distortion_B.clone()
        labels_distortion_B[attention_mask_distortion_B == 0] = -100
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train Distortion Classification task")
    ...
    # Loss arguments (distortion task only uses CE loss)
    parser.add_argument("--weight_ce", type=float, default=1.0,
                        help="Weight for cross entropy loss")
    
    # Training arguments
    parser.add_argument("--output_dir", type=str, default="outputs/distortion_classifier",
    ...
    parser.add_argument("--gradient_checkpointing", action="store_true", default=False,
                        help="Enable gradient checkpointing (default: False for distortion task)")
...
def main():
    ...
    # Create datasets with distortion labels enabled
    print("Creating pair dataset for distortion classification training...")
    train_dataset = IQAPairDataset(
        dataset_paths=dataset_paths,
        processor=processor,
        tokenizer=tokenizer,
        split="training",
        use_scene_labels=False,  # Disable scene labels
        use_distortion_labels=True,  # Enable distortion labels
    )
    ...
    model = IQAModelWrapper(
        ...
        weight_kl=0.0,  # No KL loss for distortion task
        weight_fidelity=0.0,  # No fidelity loss for distortion task
    )
    ...
    # Create trainer with distortion-specific compute_loss
    class DistortionTrainer(IQATrainer):
        def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
            """
            Compute loss for distortion classification task only.
            """
            # Forward pass with distortion inputs
        ...
        def prediction_step(self, model, inputs, prediction_loss_only: bool, ignore_keys=None):
            """
            Override prediction_step for distortion task - only compute loss, no metrics.
            """
            ...
            # Return (loss, None, None) - no logits or labels needed for distortion task
    ...
    print("\n" + "=" * 60)
    print("Distortion classification training completed!")
    print("=" * 60)
